...
#pragma omp master
  for (int i=0; i < GRAIN; i++)
    for (int j=0; j < GRAIN; j++)
#pragma omp task firstprivate (i,j) shared(cont) 			\
  depend(in: cellulem[GRAIN-i][GRAIN-j-1],cellulem[GRAIN-i-1][GRAIN-j])\
  depend(out : cellulem[GRAIN-i-1][GRAIN-j-1])
      lancer_monte (GRAIN-i-1, GRAIN-j-1);
...
#pragma omp master
  for (int i=0; i < GRAIN; i++)
    for (int j=0; j < GRAIN; j++)
#pragma omp task firstprivate (i,j)  shared(cont)	\
  depend(in:celluled[i-1][j],celluled[i][j-1])	\
  depend(out:celluled[i][j])
      lancer_descente (i, j);
  }
